...
group by {self.first_column}, {self.second_column}
union all
select 0, {table}.{self.first_column}, toUInt64(round(count() / uniq({table}.{self.second_column}))) as total_count
from {table}
    prewhere timestamp > toUnixTimestamp(now()) - {time_offset}
where {table}.{self.first_column} != 0
group by {table}.{self.first_column}"""
